...
def test_grounded_perception_and_reasoning():
    ...
    # Check states
    states = [e["next_state"] for e in events if e["event_type"] == EventType.state_transition.value]
    print(f"States: {states}")
    
    if RuntimeState.awaiting_approval.value in states:
        print("Run paused for approval as expected.")
        approval_event = [e for e in events if e["event_type"] == EventType.approval_requested.value][0]
        ...
        token = "test-token"
        grant = ApprovalGrant(approval_id=approval_id, token=token, decision=ApprovalDecision.granted)
    ...
    selected = [e for e in events if e["event_type"] == EventType.action_selected.value][0]
    assert selected["payload"]["kind"] == "store_note"
    
    started = [e for e in events if e["event_type"] == EventType.execution_started.value][0]
